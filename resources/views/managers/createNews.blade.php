...
<head>
    ...
    <style>
        ...
        /* Điều chỉnh kiểu dáng CKEditor để phù hợp với Tailwind */
...
<div class="bg-white p-6 md:p-8 rounded-lg shadow-xl w-full max-w-4xl border border-gray-200">
    <h1 class="text-2xl font-semibold mb-6 text-gray-800">Soạn bài viết mới</h1>
    ...
    <form action="{{ route('news.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf <div>
            ...
            <input type="text" id="title" name="title" value="{{ old('title') }}" placeholder="Nhập tiêu đề bài viết tại đây..."
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select id="isHot" name="isHot"
                        class="shadow-sm border border-gray-300 rounded-md w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 ease-in-out">
                    <option value="0" {{ old('isHot', '0') == '0' ? 'selected' : '' }}>Không</option>
                    <option value="1" {{ old('isHot') == '1' ? 'selected' : '' }}>Có</option>
                </select>
            </div>
        </div>
        <div>
            ...
            <div class="flex items-center space-x-4">
                {{-- Khu vực upload file - Đã làm nhỏ lại và luôn hiển thị --}}
                ...
                {{-- Khu vực hiển thị ảnh đã chọn - Luôn hiển thị (ban đầu src rỗng hoặc ảnh placeholder) --}}
                <div id="thumbnail-preview-container" class="flex-1 border border-gray-300 rounded-lg overflow-hidden flex items-center justify-center p-2 min-h-[128px]">
                    <img id="thumbnail-preview-img" src="" alt="Thumbnail Preview" class="w-full h-auto object-contain max-h-40">
                    {{-- Placeholder text nếu không có ảnh --}}
                    <p id="no-image-text" class="text-gray-400 text-sm italic @if(old('thumbNailUrl')) hidden @endif">Chưa có ảnh nào được chọn</p>
                </div>
            </div>
        </div>
        <div>
...
<script>
    ...
    const noImageText = document.getElementById('no-image-text'); // Lấy element text placeholder

    // Hàm hiển thị preview ảnh
    function showImagePreview(file) {
        if (!file) {
            // Nếu không có file, reset preview về trạng thái không ảnh
    ...
    fileInput.addEventListener('change', (e) => {
        ...
        if (files.length > 0) {
            ...
            showImagePreview(null); // Reset preview
        }
    });
